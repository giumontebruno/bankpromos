...
@register_scraper("py_sudameris")
class SudamerisPromotionsScraper(BasePublicScraper):
    BENEFITS_URL = "https://www.bancosudameris.com.py/beneficios"
    ...
    def _scrape_promotions(self) -> List[PromotionModel]:
        ...
        self._save_debug_screenshot("sudameris_main")
    ...
    def _extract_from_page(self) -> List[PromotionModel]:
        ...
        for card in cards:
            try:
                if self._is_noise_element(card):
                    continue
    ...
    def _is_noise_element(self, card) -> bool:
        try:
            tag_name = card.evaluate("el => el.tagName")
            if tag_name.upper() in ("NAV", "FOOTER", "HEADER"):
                return True
        except Exception:
            pass
        return False
