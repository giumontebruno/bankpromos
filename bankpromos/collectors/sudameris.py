...
class SudamerisCollector(BaseCollector):
    bank_id = "sudameris"
    display_name = "Banco Sudameris"

    def discover_sources(self) -> List[Source]:
        sources = []
        
        pdf_sources = get_sources_for_bank("sudameris")
                ...
                sources.append(
                    Source(
                        ...
                        metadata={
                            ...
                            "size": ps.get("size", 0),
                            "bank_detected": ps.get("bank", "sudameris"),
        ...
        sources.append(
            Source(
                ...
                url="https://www.bancosudameris.com.py/beneficios",
                title="Sudameris Beneficios HTML",
                metadata={"source": "html"},
            )
        )

        logger.info(f"Sudameris: discovered {len(sources)} sources")
        return sources
    ...
    def collect(self, sources: Optional[List[Source]] = None) -> List[Promotion]:
        ...
        if html_sources:
            try:
                ...
                logger.warning(f"HTML scrape error: {e}")

        if all_review_items:
            save_review_items(all_review_items)
            logger.info(f"Sudameris: saved {len(all_review_items)} review items")

        logger.info(f"Sudameris: collected {len(all_promos)} promos")
        return all_promos
    ...
    def _parse_pdf(self, source: Source) -> tuple[List[Promotion], List[dict]]:
        ...
        for rp in raw_promos:
            if not rp.merchant_name and not rp.discount_percent and not rp.installment_count:
                continue
            ...
            is_fake = any(fake in merchant_lower for fake in [
                ...
                "los meses con", "disfrut", "beneficio del"
            ])

            is_bank = merchant_lower in ("sudameris", "py_sudameris", "santander", "py_santander", "ueno", "py_ueno")
            ...
            pattern_key = f"sudameris:{filename}:{page_num}:{raw_text[:100].lower().strip()}"
    ...
    def _scrape_html(self) -> List[Promotion]:
        ...
        try:
            scraper = get_scraper("py_sudameris", debug_mode=False)
            ...
            for rp in raw_promos:
                promo = Promotion(
                    ...
                    metadata={"collector": "sudameris", "scraper": "py_sudameris"},
                )
                promos.append(promo)
        except Exception as e:
...
def get_collector(bank_id: str = "sudameris") -> Optional[BaseCollector]:
    if bank_id in ["sudameris", "py_sudameris"]:
        return SudamerisCollector()
    return None


def collect_sudameris() -> CollectorResult:
    collector = SudamerisCollector()
